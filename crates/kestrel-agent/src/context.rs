...
//! Uses [`PromptAssembler`] from kestrel-learning to combine [`PromptSection`]
...
use kestrel_learning::prompt::{PromptAssembler, PromptSection, ToolInfo};
...
impl<'a> ContextBuilder<'a> {
    ...
    fn build_identity_content(&self) -> String {
        let name = self.config.name.as_deref().unwrap_or("Kestrel");
        format!(
            "You are {}, an AI assistant powered by kestrel. \
...
mod tests {
    ...
    fn test_build_identity_default() {
        ...
        assert!(identity.contains("Kestrel"));
    ...
    fn test_skill_index_with_entries() {
        let config = Config::default();
        let entries = vec![
            kestrel_learning::prompt::SkillIndexEntry {
    ...
    fn test_enriched_context_section_order() {
        ...
        let entries = vec![kestrel_learning::prompt::SkillIndexEntry {
