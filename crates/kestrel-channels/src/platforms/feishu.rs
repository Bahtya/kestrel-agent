...
// Protobuf definitions generated by prost from proto/pbbp2.proto.
mod pbbp2 {
...
async fn discover_ws_endpoint(
    ...
    let ws_url = data.URL.context("WS endpoint response missing URL")?;

    info!(
        "[feishu:ws] discovered endpoint: {}...{}",
        &ws_url[..50.min(ws_url.len())],
        if ws_url.len() > 50 { "..." } else { "" }
    );
    Ok(ws_url)
}
...
async fn run_ws_event_loop(
    ...
    while running.load(Ordering::Relaxed) {
        if let Err(e) =
            ws_connect_and_listen(&app_id, &app_secret, &client, &handler, &running).await
        {
                            ...
                            match msg_type.as_str() {
                                MSG_TYPE_PING => {
                                    ...
                                    pong.encode(&mut pong_buf)
                                        .map_err(|e| {
                                            warn!("[feishu:ws] failed to encode pong: {e}")
                                        })
                                        .ok();
                                    if let Err(e) =
                                        write.send(WsMessage::Binary(pong_buf.into())).await
                                    {
                    ...
                    FRAME_TYPE_DATA => {
                        ...
                        if let Some(msg_type) = get_header_value(&frame, HEADER_TYPE) {
                            if msg_type == MSG_TYPE_EVENT {
                                ...
                                let resp_header = pbbp2::Header {
                                    key: HEADER_TYPE.to_string(),
                                    value: "response".to_string(),
                                };
                                let resp_body = serde_json::json!({"code": 0});
                                let resp = pbbp2::Frame {
                                    service: frame.service,
                                    method: FRAME_TYPE_DATA,
                                    seq_id: frame.seq_id,
                                    log_id: frame.log_id,
                                    headers: vec![resp_header],
                                    payload: Some(resp_body.to_string().into_bytes()),
                                ...
                                if resp.encode(&mut resp_buf).is_ok() {
                                    if let Err(e) =
                                        write.send(WsMessage::Binary(resp_buf.into())).await
                                    {
                ...
                if let Ok(wf) = serde_json::from_str::<serde_json::Value>(&text) {
                    if let Some(cmd) = wf.get("cmd").and_then(|v| v.as_str()) {
                        if cmd == "ping" {
                            ...
                            if let Err(e) =
                                write.send(WsMessage::Text(pong.to_string().into())).await
                            {
