...
impl FeishuChannel {
    ...
    fn spawn_ws_loop(
        ...
        tokio::spawn(async move {
            run_ws_event_loop(app_id, app_secret, client, handler, running).await;
        });
    }
}

// ---------------------------------------------------------------------------
// Feishu WebSocket long-connection protocol (protobuf-based)
// ---------------------------------------------------------------------------

/// Endpoint discovery response from `/callback/ws/endpoint`.
#[derive(Debug, Deserialize)]
...
/// Frame type constants (from Feishu WS protocol).
const FRAME_TYPE_CONTROL: i32 = 0;
const FRAME_TYPE_DATA: i32 = 1;

/// Message type constants for CONTROL frames.
...
/// Discover the WebSocket endpoint URL via the Feishu endpoint API.
///
/// Feishu WS protocol requires first calling `/callback/ws/endpoint` to obtain
/// a dynamic WebSocket URL with embedded auth parameters.
async fn discover_ws_endpoint(
    client: &reqwest::Client,
    app_id: &str,
    app_secret: &str,
) -> Result<String> {
    let url = format!("{}{}", FEISHU_BASE_URL.replace("/open-apis", ""), FEISHU_WS_ENDPOINT_PATH);
    let body = serde_json::json!({
        "AppID": app_id,
        "AppSecret": app_secret,
    });

    info!("[feishu:ws] discovering WS endpoint at {}", url);
    let resp = client
        .post(&url)
        .header("Content-Type", "application/json")
    ...
    if endpoint_resp.code != 0 {
        anyhow::bail!(
            "WS endpoint discovery failed: code={}, msg={:?}",
            endpoint_resp.code,
            endpoint_resp.msg
        );
    }
    ...
    let data = endpoint_resp
        .data
        .context("WS endpoint response missing data")?;

    let ws_url = data
        .URL
        .context("WS endpoint response missing URL")?;

    info!("[feishu:ws] discovered endpoint: {}...{}", &ws_url[..50.min(ws_url.len())], if ws_url.len() > 50 { "..." } else { "" });
    Ok(ws_url)
}

/// Build a new protobuf ping Frame.
fn new_ping_frame(service_id: u64) -> pbbp2::Frame {
    let mut header = pbbp2::Header::default();
    header.key = HEADER_TYPE.to_string();
    header.value = MSG_TYPE_PING.to_string();

    let mut frame = pbbp2::Frame::default();
    frame.service = service_id as i32;
...
/// Extract a header value from a protobuf Frame.
fn get_header_value(frame: &pbbp2::Frame, key: &str) -> Option<String> {
    frame
        .headers
        .iter()
        .find(|h| h.key == key)
...
///
/// Uses the Feishu WS long-connection protocol:
/// 1. Discover WS endpoint via `/callback/ws/endpoint`
/// 2. Connect to dynamic WS URL
/// 3. Communicate using protobuf Frame encoding
async fn run_ws_event_loop(
    ...
    while running.load(Ordering::Relaxed) {
        if let Err(e) = ws_connect_and_listen(&app_id, &app_secret, &client, &handler, &running).await {
...
/// Connect to Feishu WebSocket via endpoint discovery, then process protobuf frames.
async fn ws_connect_and_listen(
    ...
    // Step 1: Discover the dynamic WS endpoint URL.
    let ws_url = discover_ws_endpoint(client, app_id, app_secret).await?;

    // Extract service_id from the URL query params.
    let service_id = url::Url::parse(&ws_url)
        .ok()
        .and_then(|u| {
            u.query_pairs()
                .find(|(k, _)| k == "service_id")
                .map(|(_, v)| v.parse::<u64>().unwrap_or(1))
        })
        .unwrap_or(1);

    // Step 2: Connect to the dynamic WS URL.
    info!("[feishu:ws] connecting to discovered endpoint");
    let request = tokio_tungstenite::tungstenite::client::IntoClientRequest::into_client_request(&ws_url)
        .context("Failed to create WS request from URL")?;
    let (ws_stream, _) = tokio_tungstenite::connect_async(request)
        .await
        .context("Feishu WebSocket connect failed")?;

    info!("[feishu:ws] connected, processing frames");
    let (mut write, mut read) = ws_stream.split();

    // Step 3: Process incoming frames.
        ...
        match msg {
            WsMessage::Binary(data) => {
                ...
                let frame = match pbbp2::Frame::decode(data.as_ref()) {
                    Ok(f) => f,
                    Err(e) => {
                        debug!("[feishu:ws] failed to decode protobuf frame: {e}");
                        continue;
                    }
                };
                ...
                match frame.method {
                    FRAME_TYPE_CONTROL => {
                        // Control frame: ping/pong/config.
                        if let Some(msg_type) = get_header_value(&frame, HEADER_TYPE) {
                            match msg_type.as_str() {
                                MSG_TYPE_PING => {
                                    ...
                                    pong.encode(&mut pong_buf)
                                        .map_err(|e| warn!("[feishu:ws] failed to encode pong: {e}"))
                                        .ok();
                                    if let Err(e) = write.send(WsMessage::Binary(pong_buf.into())).await {
                                        warn!("[feishu:ws] pong send error: {e}");
                                        break;
                                    }
                                }
                                MSG_TYPE_PONG => {
                                ...
                                other => {
                                    debug!("[feishu:ws] unknown control type: {other}");
                                }
                            }
                        }
                    }
                    FRAME_TYPE_DATA => {
                        // Data frame: event message.
                        if let Some(msg_type) = get_header_value(&frame, HEADER_TYPE) {
                            if msg_type == MSG_TYPE_EVENT {
                                ...
                                // Acknowledge the data frame with a response.
                                let mut resp = pbbp2::Frame::default();
                                resp.service = frame.service;
                                resp.method = FRAME_TYPE_DATA;
                                resp.SeqID = frame.SeqID;
                                resp.LogID = frame.LogID;
                                let mut resp_header = pbbp2::Header::default();
                                resp_header.key = HEADER_TYPE.to_string();
                                resp_header.value = "response".to_string();
                                resp.headers.push(resp_header);

                                // Response body: {"code":0}
                                let resp_body = serde_json::json!({"code": 0});
                                resp.payload = resp_body.to_string().into_bytes();
                                resp.payload_type = "application/json".to_string();

                                let mut resp_buf = Vec::with_capacity(resp.encoded_len());
                                if resp.encode(&mut resp_buf).is_ok() {
                                    if let Err(e) = write.send(WsMessage::Binary(resp_buf.into())).await {
                                        debug!("[feishu:ws] ack send error: {e}");
                                    }
                                }
                            } else {
                    ...
                    other => {
                        debug!("[feishu:ws] unknown frame method: {other}");
                    }
                }
            }
            WsMessage::Ping(data) => {
            ...
            WsMessage::Text(text) => {
                // Some older Feishu WS versions still send JSON frames.
                // Try to parse as JSON fallback.
                debug!("[feishu:ws] received text frame ({} bytes)", text.len());
                if let Ok(wf) = serde_json::from_str::<serde_json::Value>(&text) {
                    if let Some(cmd) = wf.get("cmd").and_then(|v| v.as_str()) {
                        if cmd == "ping" {
                            // Respond with pong JSON.
                            let pong = serde_json::json!({"cmd": "pong"});
                            if let Err(e) = write.send(WsMessage::Text(pong.to_string().into())).await {
                                warn!("[feishu:ws] json pong send error: {e}");
                            }
                        }
                    }
                }
