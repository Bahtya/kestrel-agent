...
///
/// ```
/// use kestrel_channels::platforms::telegram::InlineKeyboardBuilder;
///
...
///
/// ```no_run
/// use kestrel_channels::platforms::telegram::{
...
impl TelegramChannel {
    ...
    fn build_client(proxy_config: Option<&str>) -> reqwest::Client {
                ...
                match reqwest::Proxy::all(url) {
                    Ok(proxy) => {
                        ...
                        match b.build() {
                            ...
                            Err(e) => {
                                warn!("Telegram: failed to build SOCKS5 client ({}), falling back to direct", e);
                    ...
                    Err(e) => {
                        warn!(
                            "Telegram: invalid SOCKS5 proxy '{}' ({}), falling back to direct",
                            url, e
                        );
                        reqwest::Client::builder().dns_resolver(dns)
                    }
                ...
                match (http_proxy, https_proxy) {
                    (Ok(hp), Ok(sp)) => {
                        ...
                        match b.build() {
                            Ok(client) => return client,
                            Err(e) => {
                                warn!("Telegram: failed to build HTTP proxy client ({}), falling back to direct", e);
                                reqwest::Client::builder().dns_resolver(dns)
                            }
                        }
                    }
                    (http_res, https_res) => {
                        if let Err(e) = http_res {
                            warn!("Telegram: invalid HTTP proxy config for '{}' ({}), falling back to direct", url, e);
                        } else if let Err(e) = https_res {
                            warn!("Telegram: invalid HTTPS proxy config for '{}' ({}), falling back to direct", url, e);
                        }
                        reqwest::Client::builder().dns_resolver(dns)
                    }
                }
        ...
        builder.build().unwrap_or_else(|e| {
            warn!(
                "Telegram: failed to build direct client ({}), using default",
                e
            );
            reqwest::Client::new()
        })
...
mod tests {
    ...
    async fn test_settings_callback_handler_dispatches() {
        ...
        let _env = crate::test_support::EnvVarGuard::set("KESTREL_HOME", home.path());
        let mut router = CallbackRouter::new();
        let session_keys = Arc::new(ParkMutex::new(Vec::new()));
        TelegramChannel::register_default_handlers(&mut router, &session_keys);
