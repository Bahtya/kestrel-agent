...
use nanobot_learning::event::{LearningEvent, LearningEventBus, SkillOutcome};
use nanobot_learning::prompt::PromptAssembler;
use nanobot_memory::types::{MemoryCategory, MemoryEntry, MemoryQuery, ScoredEntry};
...
fn make_providers(mock: Arc<MockProvider>) -> nanobot_providers::ProviderRegistry {
...
async fn test_self_evolution_provider_error() {
    ...
    let mut providers = nanobot_providers::ProviderRegistry::new();
