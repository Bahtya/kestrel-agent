...
fn test_load_config_from_file_with_env_vars() {
    ...
    std::env::set_var("TEST_NANOBOT_TG_TOKEN", "999888:XYZ");
    std::env::set_var("TEST_NANOBOT_OPENAI_KEY", "sk-test-key-12345");

    let yaml_content = r#"
_config_version: 4
providers:
  openai:
    api_key: ${TEST_NANOBOT_OPENAI_KEY}
channels:
  telegram:
    token: ${TEST_NANOBOT_TG_TOKEN}
    enabled: true
agent:
  model: gpt-4o
"#;
    ...
    std::env::remove_var("TEST_NANOBOT_TG_TOKEN");
    std::env::remove_var("TEST_NANOBOT_OPENAI_KEY");
}
