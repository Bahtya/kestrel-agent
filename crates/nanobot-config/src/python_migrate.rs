...
//!
//! Reads Python JSON/YAML configs, converts to nanobot-rs [`Config`],
//! validates the result, and reports mapped/unmapped/ignored fields.
//!
//! ## Usage
//!
//! ```ignore
//! use nanobot_config::python_migrate::{migrate_from_python, MigrationOptions};
//! use std::path::Path;
//!
...
//!     ..Default::default()
//! };
//! let result = migrate_from_python(Path::new("~/.nanobot"), &opts).unwrap();
...
pub struct MigrationOptions {
    ...
    /// Output path for the nanobot-rs config.yaml.
    /// If `None`, writes to `<NANOBOT_RS_HOME>/config.yaml`.
...
pub fn migrate_from_python(
    ...
    if options.fill_defaults {
        ...
        report.add_note("_config_version", "set to 4 (nanobot-rs current)");
    }
    ...
    if let Some(ref dc) = discord_py {
        ...
        if dc.group_policy.is_some() {
            report.add_unmapped(
                ...
                "no nanobot-rs equivalent (group behavior configured differently)",
            );
        }
    }
...
mod tests {
    ...
    fn test_migrate_from_str_yaml() {
        // Python nanobot can also output YAML configs
        let yaml = r#"
providers:
  openai:
    apiKey: sk-yaml-test
...
"#;
    ...
    fn test_migrate_from_python_json_file() {
        ...
        let py_home = tmp.path().join("nanobot");
...
"#;
    ...
    fn test_migrate_from_python_no_config_file_errors() {
        let tmp = tempfile::tempdir().unwrap();
        let py_home = tmp.path().join("empty_nanobot");
        fs::create_dir_all(&py_home).unwrap();
    ...
    fn test_per_channel_yaml_discovery() {
        ...
        let tg_dir = tmp.path().join("nanobot-telegram");
