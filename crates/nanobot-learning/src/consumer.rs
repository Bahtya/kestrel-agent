...
mod tests {
    ...
    use nanobot_skill::{CompiledSkill, Skill, manifest::SkillManifestBuilder};
    ...
    impl MemoryStore for MockMemoryStore {
        ...
        async fn recall(&self, id: &str) -> MemoryResult<Option<MemoryEntry>> {
            Ok(self.entries.lock().iter().find(|entry| entry.id == id).cloned())
        }
    ...
    async fn dispatch_actions_records_insight_in_memory_store() {
        ...
        assert!(entry.content.contains("[tool_reliability] Tool 'web' is unreliable"));
    ...
    async fn dispatch_actions_treat_noop_as_noop() {
        ...
        consumer
            .dispatch_actions(vec![LearningAction::NoOp])
            .await;
