...
> 目标：识别 Hermes Agent 系统中的故障模式、安全漏洞、扩展性极限和数据完整性风险，为 nanobot-rust 移植提供防护清单。
...
> 基于对 `/opt/nanobot-rust/nanobot-rust/` 全部 crate 源码的逐行审阅，结合 Hermes 自我进化功能（技能学习、记忆管理、上下文压缩、自我审查）的移植需求，识别所有迁移风险。

---

### 1. 并发安全风险矩阵

#### 1.1 现有 nanobot-rust 并发模型概述

nanobot-rust 采用 **tokio async + Arc 共享引用** 模型，核心组件的并发安全现状：
...
- nanobot-rust 的 `MemoryStore` **完全没有这些保护**
...
- nanobot-rust 的 `SkillsLoader` 只负责**读取**技能，没有技能**创建**功能
...
nanobot-rust 已实现了 `ContextBudget`（`agent/context_budget.rs`），默认分配：
...
nanobot-rust 的 `skills_prompt()`（`agent/skills.rs:222-232`）：
```rust
...
**关键发现**：nanobot-rust 将技能的**完整 instructions** 注入系统提示，而非仅注入名称+描述索引！

- Hermes 的 `prompt_builder.py:793-796` 只注入技能**索引**（名称 + 一行描述），按需加载
- nanobot-rust 注入所有技能的**完整内容** → Token 开销远大于 Hermes
...
nanobot-rust 的 `compaction.rs` 使用**本地文本摘要**（非 LLM 调用），策略：
...
- **当技能 + 笔记 + 自我审查结果 + 消息历史总 token 超出预算时**：
  - nanobot-rust 的 `prune_messages()` 会丢弃旧消息
...
nanobot-rust 的 `SkillsLoader`（`agent/skills.rs`）：
...
| 攻击面 | Hermes 保护 | nanobot-rust 保护 | 移植风险 |
|--------|------------|------------------|---------|
...
虽然当前 nanobot-rust 没有社区技能下载功能，但自我进化功能的移植应预先考虑：
...
#### 5.3 nanobot-rust 安全模块的局限性

**`nanobot-security` crate 仅提供 SSRF 保护**：
