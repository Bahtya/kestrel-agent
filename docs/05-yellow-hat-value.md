...
每一层都是可选的、独立的、可扩展的。这种模块化设计让 nanobot-rust 可以逐步实现。
...
> 分析者：Yellow Hat（价值与机会视角）
> 分析对象：nanobot-rust 现状 × Hermes 自演化特性
> 方法：逐 crate 源码审计 + 功能对照 + 成本精确估算

### 1. 现状差距热力图

| 功能 | nanobot-rust 现状 | 差距程度 | 用户影响 |
|------|------------------|---------|---------|
...
**关键发现**：nanobot-rust **远比预期成熟**。72,566 行代码、745 个测试、13 个 crate。核心基础设施（记忆、技能、压缩、子代理）已经完整实现。真正的差距不在于"功能缺失"，而在于**集成断裂**——各组件存在但未被 system prompt 和 LLM 引导串联起来。

### 2. 用户价值排序（重新评估）

基于 nanobot-rust 的轻量级 Rust agent 定位，优先级与 Hermes 不同：
...
| **5** | Prompt 注入扫描 | ★★★☆☆ | 安全底线，但 nanobot-rust 用户群偏向技术用户，风险意识较高 |
...
nanobot-rust 不只是"用 Rust 重写 Hermes"——以下特性是 Python 根本做不到的：
...
```
文件：crates/nanobot-agent/src/context.rs
改动：
...
```
新增文件：crates/nanobot-agent/src/skill_index.rs
...
新增文件：crates/nanobot-tools/src/builtins/skill_view.rs
...
```
新增文件：crates/nanobot-tools/src/builtins/skill_manage.rs
...
文件：crates/nanobot-tools/src/builtins/mod.rs
  - 注册 SkillManageTool

文件：crates/nanobot-tools/src/trait_def.rs
...
```
新增文件：crates/nanobot-agent/src/context_files.rs
...
```
新增文件：crates/nanobot-security/src/injection_scan.rs
...
**2 周末里程碑**：nanobot-rust 拥有完整的自演化基础闭环——记忆串联、技能引导、运行时创建、项目感知、安全扫描。
...
1. **nanobot-rust 不是"半成品"，而是"未串联的完整系统"**：72,566 行代码、13 个 crate、745 个测试。基础设施齐全，差的是 50-80 行集成代码让 system prompt 把这些功能"介绍"给 LLM。

2. **最大的杠杆点是 system prompt 组装**：Hermes 的 `prompt_builder.py` 是整个系统的灵魂。nanobot-rust 的 `ContextBuilder` 是当前最薄弱的环节——它只组装了身份、时间、工具列表和自定义指令，却没有注入记忆、技能索引、上下文文件、安全扫描、LLM 行为指导。修复这一个文件，就能让全部已有功能生效。

3. **Rust 已经在性能敏感点上超越 Hermes**：内容哈希缓存、notify 文件监听、ContextBudget 分区剪枝、100+ 并发子代理——这些都是 Python 做不到或做不好的。

4. **2 周可以闭合自演化基础闭环**：记忆串联(0.5d) + 技能引导(2d) + 安全扫描(1.5d) + 技能创建(3d) + 项目感知(2d) = 9 天。到第 10 天，nanobot-rust 就是一个有记忆、有技能、能学习、能改进、懂项目的 Agent。

5. **不应照搬 Hermes 的所有功能**：Hermes 有 16+ 平台适配、外部技能目录、洞察报告等——这些对 nanobot-rust 的轻量级定位是噪声，不是信号。专注于自演化核心闭环。
