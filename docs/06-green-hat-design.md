...
nanobot-rust:     Skill.toml + 模板 → 编译期验证 → 类型安全的执行计划 → 零拷贝注入
```
...
## 六、与 nanobot-agent 的集成设计

### 6.1 模块结构

```
crates/nanobot-agent/src/
├── lib.rs                      # 公开导出
├── evolution/                   # 自进化模块
│   ├── mod.rs                  # 模块根
...
registry.load_from_dir("~/.nanobot/skills/").await?;
...
| Hermes (Python) | nanobot-rust (本设计) | 优势 |
|-----------------|----------------------|------|
...
1. **类型系统替代运行时检查**：Hermes 用字符串和 JSON 在运行时传递一切；nanobot-rust 用 trait、枚举、newtype 在编译时保证正确性。

2. **编译替代解释**：Hermes 每次都重新解析 YAML、重建 prompt；nanobot-rust 编译一次，运行时零开销。

3. **事件驱动替代轮询**：Hermes 靠 LLM 在对话中自省；nanobot-rust 用事件总线持续学习，不需要额外 LLM 调用。

4. **分层存储替代单文件**：Hermes 的 2KB MEMORY.md 能存的信息极其有限；nanobot-rust 的三层缓存既保留速度又突破容量限制。

5. **异步原生**：Hermes 的学习发生在对话内（增加延迟）；nanobot-rust 的学习完全异步，用户无感知。
...
> 在阅读了 nanobot-rust 全部 crate 源码后，以下设计完全基于真实代码结构和风格进行精修。
> 所有代码与 nanobot-rust 现有的 `Tool` trait、`LlmProvider` trait、`CronService`、`MessageBus`、
> `SessionManager`、`ContextBuilder` 风格完全一致。

### 1. Trait 定义（完整 Rust 代码）

现有 nanobot-rust 的 trait 风格特点：
- 使用 `#[async_trait]` 宏
...
```rust
// ─── crates/nanobot-evolution/src/skill_trait.rs ──────────────────

use async_trait::async_trait;
use nanobot_core::{Message, Platform};
...
/// 核心技能 trait — 与 nanobot_tools::Tool 风格一致
#[async_trait]
...
```rust
// ─── crates/nanobot-evolution/src/memory_trait.rs ──────────────────
...
```rust
// ─── crates/nanobot-evolution/src/review_trait.rs ──────────────────
...
/// 回顾调度器 trait — 可复用 nanobot-cron 的 tick 机制
#[async_trait]
pub trait ReviewScheduler: Send + Sync {
    /// 判断是否需要回顾
    fn should_review(&self, context: &ReviewContext) -> bool;
...
└── src/
    ...
    ├── event_bus.rs                # LearningEventBus（利用现有 nanobot-bus）
...
```
nanobot-evolution
  ├── nanobot-core          # Message, Platform, error types
  ├── nanobot-bus           # MessageBus, AgentEvent（通过 emit_event 发送学习事件）
  ├── nanobot-tools         # Tool trait（新 tool 实现）、SkillLoader（复用）
  ├── nanobot-session       # SessionManager（读取会话数据）
  ├── nanobot-config        # Config（读取配置）
  └── nanobot-cron          # CronService（复用 tick 机制触发 review）
```

**不允许的依赖方向**：
- nanobot-core 不依赖 nanobot-evolution
- nanobot-session 不依赖 nanobot-evolution
- nanobot-bus 不依赖 nanobot-evolution

### 3. 消息流集成方案

#### 3.1 Bus Event 新类型定义

现有的 `AgentEvent` 枚举位于 `nanobot-bus/src/events.rs`。我们扩展它：

```rust
// ─── 在 nanobot-bus/src/events.rs 的 AgentEvent 枚举中追加 ──────
...
```rust
// ─── crates/nanobot-evolution/src/event_bus.rs ──────────────────

use nanobot_bus::{AgentEvent, MessageBus};
use std::sync::Arc;
...
现有 nanobot-session 使用 **JSONL** 格式（不是 SQLite），结构如下：
...
```rust
// ─── crates/nanobot-evolution/src/memory_store.rs ──────────────────
...
现有 `nanobot-agent/src/memory.rs` 中的 `MemoryStore` 使用 MEMORY.md 文件。新系统将：
...
```rust
// ─── crates/nanobot-evolution/src/memory_manager.rs ──────────────────

use crate::memory_trait::*;
use crate::memory_store::FileBasedMemoryStore;
use anyhow::Result;
use nanobot_agent::memory::MemoryStore as LegacyMemoryStore;
use std::sync::Arc;
...
```rust
// ─── crates/nanobot-evolution/src/context_extension.rs ──────────────────
...
修改 `nanobot-agent/src/loop_mod.rs` 中的 `process_message` 方法，在构建 system_prompt 后追加：
...
#### 6.1 基于 nanobot-cron 的 Review 调度

现有 `nanobot-cron` 使用 tick-based 调度器：`CronService::tick()` 每秒检查一次所有 active job。

我们创建一个 system cron job 来触发 self-review：

```rust
// ─── crates/nanobot-evolution/src/review_scheduler.rs ──────────────────

use crate::review_trait::*;
use crate::skill_registry::SkillRegistry;
use crate::memory_trait::MemoryStore;
use crate::event_bus::LearningEventBus;
use nanobot_cron::{CronService, CronJob, CronSchedule, CronPayload, JobState, ScheduleKind};
...
impl ReviewScheduler for HybridReviewScheduler {
    ...
    async fn review(&self, _context: &ReviewContext) -> anyhow::Result<ReviewReport> {
        ...
        // 3. 发送事件
        self.bus.emit_event(nanobot_bus::events::AgentEvent::SelfReviewCompleted {
...
三个新 tool 实现 `nanobot_tools::Tool` trait，与现有 builtins 风格一致：

#### 7.1 memory — 记忆管理 Tool

```rust
// ─── crates/nanobot-evolution/src/tools/memory_tool.rs ──────────────────

use async_trait::async_trait;
use nanobot_tools::{Tool, ToolError};
...
```rust
// ─── crates/nanobot-evolution/src/tools/skill_tool.rs ──────────────────
...
```rust
// ─── crates/nanobot-evolution/src/tools/session_search_tool.rs ──────────────────
...
```rust
// ─── crates/nanobot-evolution/src/skill_registry.rs ──────────────────

use crate::skill_trait::{ConfidenceEvent, Skill, SkillMatch, SkillOutput};
use nanobot_tools::skill_loader::SkillLoader;
use nanobot_tools::skills::Skill as DiskSkill;
use std::collections::HashMap;
use std::sync::Arc;
...
```rust
// ─── crates/nanobot-evolution/tests/integration.rs ──────────────────

use nanobot_bus::MessageBus;
use nanobot_evolution::*;
use nanobot_evolution::memory_trait::*;
use nanobot_evolution::memory_store::FileBasedMemoryStore;
use nanobot_evolution::skill_registry::SkillRegistry;
use nanobot_evolution::review_scheduler::HybridReviewScheduler;
use nanobot_evolution::tools::{MemoryTool, SkillTool, SessionSearchTool};
use nanobot_session::SessionManager;
use nanobot_tools::ToolRegistry;
use std::sync::Arc;
...
async fn test_event_bus_filters_learning_events() {
    ...
    // 发送学习相关事件
    bus.emit_event(nanobot_bus::events::AgentEvent::ToolSucceeded {
    ...
    // 发送非学习事件
    bus.emit_event(nanobot_bus::events::AgentEvent::Started {
        session_key: "test:1".to_string(),
    });
    ...
    assert!(matches!(e1, nanobot_bus::events::AgentEvent::ToolSucceeded { .. }));
    assert!(matches!(e2, nanobot_bus::events::AgentEvent::Started { .. }));
}
...
```toml
# ─── crates/nanobot-evolution/Cargo.toml ──────────────────
...
nanobot-bus = { path = "../nanobot-bus" }
nanobot-tools = { path = "../nanobot-tools" }
nanobot-session = { path = "../nanobot-session" }
nanobot-config = { path = "../nanobot-config" }
nanobot-cron = { path = "../nanobot-cron" }
nanobot-agent = { path = "../nanobot-agent" }
...
[dependencies]
# ... 现有依赖保持不变 ...
nanobot-evolution = { path = "crates/nanobot-evolution" }   # 新增
```

#### 9.4 nanobot-bus Cargo.toml — 无需变更

nanobot-bus 不需要新增依赖。新增的 `AgentEvent` 变体仅使用基本类型（String, usize, f64, u64），不需要引入新 crate。

#### 9.5 nanobot-agent Cargo.toml — 新增依赖

```toml
# ─── crates/nanobot-agent/Cargo.toml 新增 ──────────────────

[dependencies]
# ... 现有依赖保持不变 ...
nanobot-evolution = { path = "../nanobot-evolution" }   # 新增（可选依赖）
```
