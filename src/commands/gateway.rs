...
pub async fn run(config: Config, channels: Vec<String>, dangerous: bool) -> Result<()> {
    ...
    let memory_config = MemoryConfig {
        ...
        tantivy_index_path: home.join("memory").join("tantivy"),
            ...
            Ok(hot_store) => {
                ...
                if config.dream.enabled {
                    match TantivyStore::new(&memory_config).await {
                        Ok(tantivy_store) => {
                            let tiered =
                                kestrel_memory::TieredMemoryStore::new(l1, Arc::new(tantivy_store));
                            info!("Memory store initialized (HotStore L1 + TantivyStore L2)");
                            Some(Arc::new(tiered))
                        }
                        Err(e) => {
                            tracing::warn!(
                                "TantivyStore L2 init failed, falling back to L1 only: {}",
                                e
                            );
                    ...
                    info!("Memory store initialized (HotStore L1 only, TantivyStore disabled)");
                    Some(l1)
                }
            }
            Err(e) => {
...
mod tests {
    ...
    async fn test_record_insight_action_stores_memory() {
        ...
        assert!(
            !entries[0].content.is_empty(),
            "learning insight should have content"
        );
    }
